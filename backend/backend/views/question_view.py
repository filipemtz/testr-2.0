...
class QuestionViewSet(viewsets.ModelViewSet):
    ...
    def inputs_outputs(self, request, pk=None):
        ...
        serializer = InputOutputSerializer(
    ...
    def get_queryset(self):
        ...
        base_qs = (
            Question.objects.filter(section__course__teachers=user)
            | Question.objects.filter(section__course__students=user)
        ).distinct()
...
class QuestionImportAPIView(APIView):
    ...
    def post(self, request, *args, **kwargs):
        ...
        with tempfile.TemporaryDirectory() as tmpdirname:
            ...
            input_output_pairs_invisible = self.pair_input_output_files(
                tmpdirname, visible=False
            )
            ...
            if os.path.isdir(description_dir):
                ...
                for f in os.listdir(description_dir):
                    if f.endswith(".zip"):
                        ...
                        other_files.append(
                            f
                        )  # Adiciona outros arquivos à lista `other_files`
                ...
                if zip_file:
                    ...
                    with tempfile.TemporaryDirectory() as extracted_dir:
                        ...
                        input_output_pairs_visible = self.pair_input_output_files(
                            extracted_dir, visible=True
                        )
                        print(
                            "Pares de arquivos (visíveis): ", input_output_pairs_visible
                        )
    ...
    def get(self, request, question_id):
        ...
        try:
            ...
            with tempfile.TemporaryDirectory() as tmpdirname:
                ...
                input_output_pairs_invisible = InputOutput.objects.filter(
                ...
                for idx, pair in enumerate(input_output_pairs_invisible, start=1):
                    ...
                    with open(
                        os.path.join(input_dir, input_filename), "w"
                    ) as input_file:
                ...
                description_zip_path = os.path.join(
                    description_dir, f"{question_name}.zip"
                )
                with zipfile.ZipFile(
                    description_zip_path, "w", zipfile.ZIP_DEFLATED
                ) as description_zip:
                    ...
                    input_output_pairs_visible = InputOutput.objects.filter(
                    ...
                    for idx, pair in enumerate(input_output_pairs_visible, start=1):
                        ...
                        input_filename = f"test_{idx}.txt"
                        description_zip.writestr(
                            os.path.join("input", input_filename), pair.input
                        )  # ALTERADO

                        output_filename = f"test_{idx}.txt"
                        description_zip.writestr(
                            os.path.join("output", output_filename), pair.output
                        )  # ALTERADO

                    # Adiciona o arquivo PDF associado à questão (do modelo QuestionFile)
                    question_file = QuestionFile.objects.filter(
                        question=question, file_name=f"{question_name}.pdf"
                    ).first()
                    if question_file:
                        pdf_file_path = (
                            question_file.file.path
                        )  # Caminho do arquivo PDF
                        # Adiciona o arquivo PDF ao zip
                        description_zip.write(
                            pdf_file_path, os.path.basename(pdf_file_path)
                        )
                ...
                with zipfile.ZipFile(
                    final_zip_file_path, "w", zipfile.ZIP_DEFLATED
                ) as zipf:
                ...
                with open(final_zip_file_path, "rb") as zip_file:
                    ...
                    response["Content-Disposition"] = (
                        f"attachment; filename={question_name}.zip"
                    )
                    return response
