...
class GetSubmissionAPIView(APIView):
    ...
    def get(self, request, question_id):
        ...
        if not question_id:
            return Response(
                {"detail": "question_id query parameter is required."},
...
class AddSubmissionAPIView(APIView):
    ...
    def post(self, request):
        ...
        if not (is_teacher or is_student):
            return Response(
                {"detail": "You do not have permission to submit for this course."},
                status=status.HTTP_403_FORBIDDEN,
            )
...
class ResetStatusSubmissionAPIView(APIView):
    ...
    def post(self, request):
        ...
        if not (is_teacher or is_student):
            return Response(
                {
                    "detail": "You do not have permission to reset submissions for this course."
                },
                status=status.HTTP_403_FORBIDDEN,
            )
...
class GetAllStudentsSubmissionsAPIView(APIView):
    ...
    def get(self, request):
        ...
        if not question.section.course.teachers.filter(id=user.id).exists():
            return Response(
                {
                    "detail": "You do not have permission to get submissions for this course."
                },
                status=status.HTTP_403_FORBIDDEN,
            )
