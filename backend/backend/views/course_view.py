...
class CourseViewSet(viewsets.ModelViewSet):
    ...
    def sections(self, request, pk=None):
        ...
        if request.user in course.teachers.all():
            sections = course.section_set.order_by("order").all()
        else:
            sections = course.section_set.filter(visible=True).order_by("order")
    ...
    def get_queryset(self):
        ...
        return (
            Course.objects.filter(teachers=user) | Course.objects.filter(students=user)
        ).distinct()
...
class CourseRemoveTeacherAPIView(APIView):
    ...
    def post(self, request, course_id=None):
        ...
        if not course.teachers.filter(id=user.id).exists():
            return Response(
                {
                    "error": "Você não tem permissão para remover professores deste curso."
                },
        ...
        if teacher in course.teachers.all():
            ...
            return Response(
                {
                    "message": f"Professor {teacher.username} removido do curso com sucesso."
                }
            )
        else:
            return Response(
                {"error": "Este professor não está associado a este curso."},
...
class CourseAddTeacherAPIView(APIView):
    ...
    def post(self, request, course_id=None):
        ...
        if not course.teachers.filter(id=user.id).exists():
            return Response(
                {
                    "error": "Você não tem permissão para adicionar professores a este curso."
                },
                status=status.HTTP_403_FORBIDDEN,
            )
        ...
        try:
            ...
            if group.name == "teacher":
                if course.teachers.filter(id=user.id).exists():
                    return Response(
                        {"error": "Professor já está no curso."},
                    ...
                    return Response(
                        {
                            "message": f"Professor {user.username} adicionado ao curso com sucesso."
                        }
                    )
            else:
                return Response(
                    {"error": "Alunos não podem ser professores."},
                    status=status.HTTP_400_BAD_REQUEST,
                )
        except User.DoesNotExist:
            return Response(
...
class CourseAddStudentAPIView(APIView):
    ...
    def post(self, request, course_id=None):
        ...
        if not course.teachers.filter(id=user.id).exists():
            return Response(
                {"error": "Você não tem permissão para adicionar alunos a este curso."},
                status=status.HTTP_403_FORBIDDEN,
            )
        ...
        try:
            ...
            if group.name == "student":
                if course.students.filter(id=user.id).exists():
                    ...
                    return Response(
                        {
                            "message": f"Aluno {user.username} adicionado ao curso com sucesso."
                        }
                    )
            else:
                return Response(
                    {"error": "Professores não podem ser alunos."},
                    status=status.HTTP_400_BAD_REQUEST,
                )
        except User.DoesNotExist:
            return Response(
...
class CourseRegisterStudentsAPIView(APIView):
    ...
    def post(self, request, course_id=None):
        ...
        csv_columns = set(
            next(
                csv.reader(
                    [csv_file.read().decode("utf-8").splitlines()[0]], delimiter=";"
                )
            )
        )
        ...
        if not required_columns.issubset(csv_columns):
            return Response(
                {"message": "O arquivo CSV está faltando colunas obrigatórias."},
                status=status.HTTP_400_BAD_REQUEST,
            )
        ...
        for row in reader:
            ...
            if user:
                if not course.students.filter(id=user.id).exists():
                    course.students.add(user)
                    success.append(
                        f"Usuário {user.username} já existe e foi adicionado ao curso."
                    )
                else:
                ...
                if serializer.is_valid():
                    ...
                    # })
                    success.append(
                        f"Usuário {row['username']} criado e adicionado ao curso."
                    )
                else:
                    error_messages = []
                    ...
                    errors.append(
                        f"Usuário {row['username']} - " + "; ".join(error_messages)
                    )
        ...
        return Response(
            response_data,
            status=(
                status.HTTP_201_CREATED if not errors else status.HTTP_400_BAD_REQUEST
            ),
        )
...
class CourseTeachersAPIView(APIView):
    ...
    def get(self, request, course_id=None):
        ...
        if not course.teachers.filter(id=request.user.id).exists():
            return Response(
                {
                    "error": "You do not have permission to view teachers of this course."
                },
                status=403,
            )
...
class CourseCreateCopyAPIView(APIView):
    ...
    def post(self, request, course_id=None):
        ...
        if not course.teachers.filter(id=request.user.id).exists():
            return Response(
                {"error": "You do not have permission to copy this course."},
                status=status.HTTP_403_FORBIDDEN,
            )
