...
int vector_argmax(Vector *v)
{
    if (vector_size(v) == 0)
    {
        exit(printf("Error: vector_argmax: vector is empty.\n"));
    }

    int argmax = 0;
    data_type max = vector_get(v, 0);

    for (int i = 1; i < vector_size(v); i++)
    {
        if (vector_get(v, i) > max)
        {
            max = vector_get(v, i);
            argmax = i;
        }
...
int vector_argmin(Vector *v)
{
    if (vector_size(v) == 0)
    {
        exit(printf("Error: vector_argmin: vector is empty.\n"));
    }

    int argmin = 0;
    data_type min = vector_get(v, 0);

    for (int i = 1; i < vector_size(v); i++)
    {
        if (vector_get(v, i) < min)
        {
            min = vector_get(v, i);
            argmin = i;
        }
...
{
    return vector_get(v, vector_argmax(v));
}

data_type vector_min(Vector *v)
{
    return vector_get(v, vector_argmin(v));
}
